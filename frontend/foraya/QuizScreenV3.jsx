...
const QuizScreenTime = ({ route, navigation }) => {
  const { quizId } = route.params;
  const [questions, setQuestions] = useState([]);
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [selectedAnswers, setSelectedAnswers] = useState({});
  const [questionDuration, setQuestionDuration] = useState(0); // Per-question timer
...
useEffect(() => {
 const loadToken = async () => {
  ...
  // Set timer for the current question when it changes
 useEffect(() => {
  if (questions.length > 0 && questions[currentQuestionIndex]) {
    const duration = questions[currentQuestionIndex].duration;
  ...
  // Timer countdown
 useEffect(() => {
  if (!timerRunning || questionDuration == null) return;

  const interval = setInterval(() => {
    setQuestionDuration((prev) => {
      if (prev <= 1) {
  ...
  const fetchQuizData = async (quizId) => {
    try {
      ...
      const transformedQuestions = data.questions.map(q => ({
        ...
        answers: q.answers,
        duration: q.duration_minutes * 60, // convert to seconds
      }));
  ...
  const handleAnswer = async (option,token) => {
  ...
  const handleSubmitQuiz = async (token) => {
...
const fetchStudentScore = async (quizId,token) => {
  try {
    const response = await fetch(`http://10.42.0.1:7000/api/result/quiz/${quizId}/student/score`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
...
const styles = StyleSheet.create({
  ...
  questionText: { fontSize: 24, fontWeight: 'bold', marginBottom: 15,top:32,left:8 },
  ...
  button: { backgroundColor: '#184F78', padding:10,borderRadius: 10 ,top:300,width:162,
  ...
  optionContainer: {
    width: '48%', 
    height:'80%',// allows 2 per row with some space
    padding: 10,
    marginBottom: 10,
    borderRadius: 8,
    alignItems: 'center',
    borderWidth: 2,
    borderColor: '#184F78',
    backgroundColor: 'rgba(24, 79, 120, 0.05)'
  },
