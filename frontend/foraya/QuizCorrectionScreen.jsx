...
const fetchQuizCorrection = async (quiz_id, token) => {
  try {
    const response = await fetch(`http://192.168.100.30:7000/api/result/quiz-correction/${quiz_id}`,{
     method: 'GET',
      headers: {
...
useEffect(() => {
 ...
 useEffect(() => {
  const getCorrection = async () => {
    if (token) {
      const result = await fetchQuizCorrection(quiz_id, token);
      if (result) {
        setQuestions(result);
      }
    }
